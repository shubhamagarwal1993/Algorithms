...
void mergesortRecursive(int arr[], int l, int r) {
    ...
    //
    //  height of tree(h):
    //    value of each node = n/(2^h)
    //    n/(2^h) = 2 at last level
    //    Thus n = 2(2^h) => h = logn base 2
    //
...
void mergesortIterative(int arr[], int arr_size) {
    ...
    // curr_size: For current size of subarrays to be merged. curr_size varies from 1 to n/2
    // left_start: For picking starting index of left subarray to be merged

    // Merge subarrays in bottom up manner.  First merge subarrays of
    // size 1 to create sorted subarrays of size 2, then merge subarrays
    // of size 2 to create sorted subarrays of size 4, and so on.

    // divide the array into blocks of size curr_size
    // curr_size = [1, 2, 4, 8, 16, ...]
    for(int curr_size = 1; curr_size <= arr_size-1; curr_size = 2*curr_size) {
        
        // for curr_size = 1, left_start = 0, 2, 4, 6, 8
        // for curr_size = 2, left_start = 0, 4, 8
        // for curr_size = 4, left_start = 0, 8
