...
double sqrt_double(double num) {
    ...
    // define the precision, so we stop when precision is achieved
    ...
    // binary search - reduce range till precision is reached
    ...
    // If we did not find exact sqrt root value, return the approxiated value with the defined precision
    return (start+end)/2;
}
...
int main() {
    ...
    // finds closest integer sq root value - uses binary search
    cout << binary_search_floor_sq_root(int(y)) << endl;

    // uses binary search to reduce range for given precision
